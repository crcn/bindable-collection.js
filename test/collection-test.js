var BindableCollection = require(".."),
...
describe("collection-test#", function () {
  ...
  it("can initialize with a source", function (){
    var src = new BindableCollection([0]);
  ...
  it("can push an item", function () {
    var src = new BindableCollection();
    src.push(0);
    expect(src.at(0)).to.be(0);
  });
  ...
  it("can remove", function () {
    var src = new BindableCollection([0, 1]);
    src.remove(0);
  ...
  it("returns the index of a value", function () {
    var src = new BindableCollection([0, 1, 2]);
    expect(src.indexOf(1)).to.be(1);
    expect(src.indexOf(99999)).to.be(-1);
  });

  it("can filter a collection", function () {
    var src = new BindableCollection([0, 1, 2, 3, 4]),
  ...
  it("can call each value", function () {
    var src = new BindableCollection([0, 1, 2, 3]), i = 0;
    src.each(function (v) {
      expect(i++).to.be(v);
    });
  });
  ...
  it("can map values", function () {
    ...
    expect(src2[0]).to.be(1);
    expect(src2[1]).to.be(2);
  });

  it("can join values", function () {
    var src = new BindableCollection(["a", "b", "c"]);
    var src2 = src.join("+");
    expect(src2).to.be("a+b+c")
  });

  it("can bind to the length of the collection", function () {
    var src = new BindableCollection(), len;
  ...
  it("can bind to the 'empty' property of a collection", function() {
    var src = new BindableCollection(), empty;
  ...
  it("can listen for an insert on push", function (next){
    var src = new BindableCollection([1, 2, 3]);
  ...
  it("can listen for an reset", function (next) {
    var src = new BindableCollection([99]);
  ...
  it("can listen for an replace", function () {
    ...
    var src = new BindableCollection([99, 999]);
  ...
  it("can bind to the first item in an array", function (){
    var src = new BindableCollection([0, 1]), c = 0;
